...
def validate number
  ...
  number.digits.each_with_index do |value, index|
    value *= 2 if index.odd? #double the value of a digit every second digit
    value -= 9 if value >= 10

    p result = result.unshift(value)
  end

  p sum = result.reduce(:+)

  is_valid = true if sum % 10 == 0
