...
const pairWise = function ( array, sum ) {
  ...
  array.forEach((element, i) => {
    const foundIndex = array.indexOf(sum - element)

    console.log(`element: ${element} at index: ${i}`)

    if (foundIndex >= 0) {
      if (!indexList.includes(foundIndex) && !indexList.includes(i)) {
        indexList.push(i, foundIndex)
      }
    }
  });
  console.log(indexList)
  return `The result is: ${ indexList.reduce((accumlator, currentValue) => accumlator + currentValue)}`
}
