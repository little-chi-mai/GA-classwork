...
const cipher = (string, num) => {
    ...
    const codedLetters = letters.slice(num).concat(letters.slice(0, num));

    const index = string.split('').map(letter => letters.indexOf(letter));

    const codedString = index.map(i => codedLetters[i]).join('');

    return codedString;
}

console.log(cipher("joel", 3));